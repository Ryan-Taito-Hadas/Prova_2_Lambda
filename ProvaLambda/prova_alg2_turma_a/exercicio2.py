...
Objetivo:
Crie uma função chamada `analisar_argumentos` que receba um número arbitrário de argumentos utilizando 
*args e analise quais deles são números (int ou float). A função deve retornar um dicionário com as seguintes chaves:
...
lista = (10, "texto", 5.5, 3, [1,2], 8)

dic = {'maior': (lambda x: x if (isinstance(lista,(int,float)) > lista else None) )}
print (dic)
