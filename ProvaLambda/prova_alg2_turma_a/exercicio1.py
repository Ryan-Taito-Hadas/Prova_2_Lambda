...
vendas = {
  "Produto A": sum(vendas['Produto A'])/3,
  "Produto B": sum(vendas['Produto B'])/3,
  "Produto C": sum(vendas['Produto C'])/3}

def media_vendas(lista,num):
  if float(lista) > num:
    print (lista)

resultado = media_vendas(vendas.keys(),150)
print(resultado)
